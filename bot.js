...
function log(tag, msg) {
  ...
  const line = `[${ts}] [${tag.padEnd(6)}] ${msg}`;
  console.log(line);
}

// ─── GLOBAL ERROR HANDLERS (freeze prevention) ──────────────────────────────
process.on("uncaughtException", (err) => {
  log("CRASH", `uncaughtException: ${err.message}`);
  log("CRASH", err.stack || "no stack");
});
process.on("unhandledRejection", (reason) => {
  log("CRASH", `unhandledRejection: ${reason}`);
});
...
// ─── BOT (with auto-restart on polling failure) ──────────────────────────────
let bot;
...
// Watchdog: if no update in 90s, restart polling
setInterval(() => {
  const silent = Date.now() - lastUpdateAt;
  if (silent > 90000) {
    log("WATCH", `No update for ${Math.round(silent/1000)}s — restarting polling`);
...
async function fetchCromaPrice(productId, pincode) {
  ...
  try {
    ...
    if (!item) { log("API❌ ", `[${pincode}] No pricelist`); return null; }
    ...
    if (!raw) { log("API❌ ", `[${pincode}] No price field`); return null; }

    const price = parseFloat(String(raw).replace(/[^\d.]/g, ""));
    if (isNaN(price) || price <= 0) { log("API❌ ", `[${pincode}] Invalid: ${raw}`); return null; }
...
async function fetchWithRetry(productId, pincode, attempts = 3) {
  ...
  log("GIVE↑", `[${pincode}] All ${attempts} failed`);
  return null;
}
...
const timers = {};   // uid → setTimeout handle (renamed from intervals)
...
// ── Recursive timer: next run schedules only AFTER current finishes ──────────
function scheduleNext(uid) {
  timers[uid] = setTimeout(async () => {
    await checkAllPrices(uid);
    scheduleNext(uid);          // chain next only after done
  }, 30000);
}
...
function startTracking(uid) {
  ...
  scheduleNext(uid);
  log("START", `Tracking scheduled for uid=${uid}`);
}
...
// ─── ALL HANDLERS (wrapped so createBot() can re-attach) ─────────────────────
  ...
  bot.on("message", async (msg) => {
    ...
    if (state?.action === "add_url") {
      ...
      if (gotCount > 0) {
        ...
        bot.editMessageText(
          `✅ *Tracking add हुई!*\n\n📦 ${nameFromUrl}\n📍 pid=${pid}\n\n` +
          `❌ *Price अभी नहीं मिली*\nYeh product current pincodes par available nahi hai\nYa temporarily out of stock hai.\n\n` +
